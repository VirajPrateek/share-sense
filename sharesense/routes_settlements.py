...
@bp.route("/<flat_id>/settlements", methods=["POST"])
@login_required
def create_settlement(flat_id):
    ...
    db.execute(
        "INSERT INTO settlements (id, flat_id, debtor_id, creditor_id, amount, status, proposed_by) VALUES (?, ?, ?, ?, ?, 'pending', ?)",
        (settlement_id, flat_id, debtor_id, creditor_id, round(float(amount), 2), request.user_id),
    )
    db.commit()
...
@bp.route("/<flat_id>/settlements", methods=["GET"])
@login_required
def get_settlements(flat_id):
    db = get_db()
    if not is_member(db, flat_id, request.user_id):
        db.close()
        return jsonify({"error": "Access denied"}), 403
    ...
    rows = db.execute(
        """SELECT s.id, s.debtor_id, s.creditor_id, s.amount, s.status, s.created_at,
           ...
           WHERE s.flat_id = ? ORDER BY s.created_at DESC""",
        (flat_id,),
    ).fetchall()
    db.close()
...
@bp.route("/<flat_id>/settlements/<settlement_id>/confirm", methods=["POST"])
@login_required
def confirm_settlement(flat_id, settlement_id):
    db = get_db()
    if not is_member(db, flat_id, request.user_id):
        db.close()
        return jsonify({"error": "Access denied"}), 403

    settlement = db.execute("SELECT * FROM settlements WHERE id = ? AND flat_id = ?", (settlement_id, flat_id)).fetchone()
    if not settlement:
        db.close()
